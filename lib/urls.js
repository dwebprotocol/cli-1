...
//                   1          2      3        4
const VERSION_REGEX = /^(dweb:\/\/)?([^/]+)(\+[^/]+)(.*)$/i
export function parseDWebUrl (str, parseQS) {
  ...
  if (!parsed.origin) parsed.origin = `dweb://${parsed.hostname}/`
  return parsed
}
...
export function fromURLToKeyStr (url) {
  try { 
    ...
    throw new Error(`Invalid dweb:// URL, ${url}`)
  }
}

export function fromPathToDTreeKeyList (path) {
