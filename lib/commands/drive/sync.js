...
Examples:
  ...
  dweb drive sync dweb://1234..af/ ./local-folder
  dweb drive sync ./local-folder dweb://1234..af/remote-folder --no-delete
  dweb drive sync dweb://1234..af/ dweb://fedc..21/
  dweb drive sync dweb://1234...af/
  dweb drive sync ./local-folder
`

export default {
  name: 'drive sync',
  description: 'Continuously sync changes between two folders in your local filesystem or in dDrives.',
  usage: {
  ...
  command: async function (args) {
    ...
    if (!args._[1]) console.error('Creating new dDrive...')
    ...
    if (!rightArgs.isDWeb) mkdirp.sync(rightArgs.path)
    if (!ok) process.exit(0)
    console.error(live ? 'Live syncing (Ctrl+c to exit)...' : 'Syncing...')

    var statusLines = ['']
    var statusLog = statusLogger(statusLines)
    if (leftArgs.isDWeb) await setupTracker(leftArgs.fs.key.toString('hex'), statusLines, statusLog)
    if (rightArgs.isDWeb) await setupTracker(rightArgs.fs.key.toString('hex'), statusLines, statusLog)
...
function isUrl (str) {
  return str.startsWith('dweb://') || /^[0-9a-f]{64}/.test(str)
}

async function parseArgs (str) {
  if (isUrl(str)) {
    let urlp = parseDWebUrl(str)
    let drive = await DWebStruct.get(urlp.hostname, {expect: 'ddrive'})
    return { fs: drive.api, path: urlp.pathname, url: urlp, raw: str, isDWeb: true }
  }
  return { path: ''+str, raw: str, isDWeb: false }
}
...
async function createTarget (source) {
  ...
  let drive = await DWebStruct.create('ddrive')
